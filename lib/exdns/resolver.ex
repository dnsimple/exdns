defmodule Exdns.Resolver do
  ...
  def resolve(message, authority, host, question) do
    ...
    resolve(
      Exdns.Records.dns_message(message, ra: false, ad: false),
      authority,
      Exdns.Records.dns_query(question, :name),
      Exdns.Records.dns_query(question, :type),
      host
    )
  end
        ...
        records =
          List.flatten(
            Enum.map(
              ...
              custom_lookup(qname, qtype, matched_records)
            )
          )
        ...
        case ns_records = Exdns.Zone.Cache.get_delegations(Exdns.Records.dns_rr(answer, :name)) do
          ...
          _ ->
            ...
            case Exdns.Zone.Cache.get_authority(qname) do
              {:ok, soa_record} ->
                if Exdns.Records.dns_rr(soa_record, :name) ==
                     Exdns.Records.dns_rr(ns_record, :name) do
  ...
  def resolve_exact_type_match(
    ...
    if name == Exdns.Records.dns_rr(answer, :name) do
      ...
      Exdns.Records.dns_message(message,
        aa: false,
        rc: @_DNS_RCODE_NOERROR,
        authority: merge_with_authority(message, ns_records)
      )
    else
  ...
  def resolve_no_exact_type_match(
        message,
        qtype,
        _host,
        _cname_chain,
    ...
    case qtype do
      ^any_type ->
        Exdns.Records.dns_message(message, aa: true, authority: authority_records)
      ...
      _ ->
        case {referral_records, exact_type_matches} do
          ...
          {[], _} ->
            Exdns.Records.dns_message(message,
              aa: true,
              answers: merge_with_answers(message, exact_type_matches)
            )
  ...
  def resolve_exact_match_referral(
        message,
        qtype,
        matched_records,
        referral_records,
    ...
    case qtype do
      ^any_type ->
        Exdns.Records.dns_message(message, aa: true, answers: matched_records)

      ^ns_type ->
        Exdns.Records.dns_message(message, aa: true, answers: referral_records)

      ^soa_type ->
        Exdns.Records.dns_message(message, aa: true, answers: authority_records)
  ...
  def resolve_exact_match_with_cname(
        message,
        qtype,
        host,
        cname_chain,
      ...
      _ ->
        if Enum.member?(cname_chain, List.last(cname_records)) do
          ...
          restart_query(
            Exdns.Records.dns_message(message,
              aa: true,
              answers: Exdns.Records.dns_message(message, :answers) ++ cname_records
            ),
            name,
            qtype,
            host,
            cname_chain ++ cname_records,
            zone,
            Exdns.Zone.Cache.in_zone?(name)
          )
        end
  ...
  def restart_delegated_query(message, name, qtype, host, cname_chain, zone, in_zone) do
    if in_zone do
      ...
      resolve(
        ...
        name,
        qtype,
        Exdns.Zone.Cache.find_zone(name, zone.authority),
        host,
        cname_chain
      )
    end
  ...
  def best_match_resolution(message, qname, qtype, host, cname_chain, best_match_records, zone) do
    # NS lookup
    referral_records = Enum.filter(best_match_records, Exdns.Records.match_type(@_DNS_TYPE_NS))
  ...
  def resolve_best_match_with_wildcard(
        message,
        qname,
        qtype,
        host,
  ...
  def resolve_best_match_with_wildcard(
    ...
    Exdns.Records.dns_message(message,
      aa: true,
      answers: merge_with_answers(message, type_matches)
    )
  end

  def resolve_best_match_with_wildcard_cname(
        message,
        _qname,
        qtype,
        host,
  ...
  def resolve_best_match_referral(
        message,
        _qname,
        qtype,
        _host,
    ...
    case {qtype, cname_chain, authority} do
      {_, _, []} ->
        Exdns.Records.dns_message(message,
          aa: false,
          authority: merge_with_authority(message, referral_records)
        )
  ...
  def best_match(qname, [_ | rest], zone) do
    best_match(
      ...
      rest,
      zone,
      Exdns.Zone.Cache.get_records_by_name(:dns.labels_to_dname(["*"] ++ rest))
    )
  end
  ...
  def filter_records(records, [{handler, _} | rest]),
    do: filter_records(handler.filter(records), rest)

  def rewrite_soa_ttl(message),
    do: rewrite_soa_ttl(message, Exdns.Records.dns_message(message, :authority), [])

  def rewrite_soa_ttl(message, [], new_authority),
    do: Exdns.Records.dns_message(message, authority: new_authority)

  def rewrite_soa_ttl(message, [r | rest], new_authority) do
    rewrite_soa_ttl(
      message,
      rest,
      new_authority ++ [Exdns.Records.minimum_soa_ttl(r, Exdns.Records.dns_rr(r, :data))]
    )
  end
    ...
    case record_names do
      ...
      _ ->
        ...
        case records do
          ...
          _ ->
            Exdns.Records.dns_message(message, additional: merge_with_additional(message, records))
        end
    end
  end
  ...
  def requires_additional_processing([answer | rest], requires_additional) do
    names =
      case Exdns.Records.dns_rr(answer, :data) do
        data when Record.is_record(data, :dns_rrdata_ns) ->
          [Exdns.Records.dns_rrdata_ns(data, :dname)]

        data when Record.is_record(data, :dns_rrdata_mx) ->
          [Exdns.Records.dns_rrdata_mx(data, :exchange)]
