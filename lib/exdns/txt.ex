defmodule Exdns.Txt do
  ...
  def parse(_, [], tokens, current_token, true), do: tokens ++ [current_token]

  def parse(string, [c | rest], tokens, current_token, escaped),
    do: parse_char(string, c, rest, tokens, current_token, escaped)
  ...
  defp parse_char(string, ?", rest, tokens, current_token, false),
    do: parse(string, rest, tokens ++ [split(current_token)], false)

  defp parse_char(string, ?", rest, tokens, current_token, true),
    do: parse(string, rest, tokens, current_token ++ [?"], false)

  defp parse_char(string, ?\\, rest, tokens, current_token, false),
    do: parse(string, rest, tokens, current_token, true)

  defp parse_char(string, ?\\, rest, tokens, current_token, true),
    do: parse(string, rest, tokens, current_token ++ [?\\], false)

  defp parse_char(string, c, rest, tokens, current_token, _),
    do: parse(string, rest, tokens, current_token ++ [c], false)
