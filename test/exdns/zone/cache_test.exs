defmodule Exdns.Zone.CacheTest do
  ...
  setup do
    ...
    soa_record =
      Exdns.Records.dns_rr(
        ...
        data: Exdns.Records.dns_rrdata_soa()
      )

    ns_record =
      Exdns.Records.dns_rr(
        name: name,
        type: :dns_terms_const.dns_type_ns(),
        data: Exdns.Records.dns_rrdata_ns(dname: "ns1.example.com")
      )

    cname_record =
      Exdns.Records.dns_rr(
        name: "www.#{name}",
        type: :dns_terms_const.dns_type_cname(),
        data: Exdns.Records.dns_rrdata_cname(dname: "something.com")
      )
    ...
    zone = %Exdns.Zone{
      name: name,
      authority: soa_record,
      records: records,
      records_by_name: Exdns.Zone.Parser.build_named_index(records)
    }

    Exdns.Zone.Cache.put_zone(name, zone)

    {:ok,
     %{:zone => zone, delegation: ns_record, authority: soa_record, cname_record: cname_record}}
  end
  ...
  test "find zone", %{zone: zone} do
    assert Exdns.Zone.Cache.find_zone(zone.name) == %{
             zone
             | records: [],
  ...
  test "get zone", %{zone: zone} do
    assert Exdns.Zone.Cache.get_zone(zone.name) ==
             {:ok, %{zone | records: [], records_by_name: :trimmed}}
  end
