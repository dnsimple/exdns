defmodule Exdns.Zone.ParserTest do
  ...
  test "json to zone with no SHA" do
    ...
    assert zone.records_by_name == %{
             name => Enum.map(records, &Exdns.Zone.Parser.json_record_to_rr/1)
           }
  end
  ...
  test "json record to A RR" do
    ...
    rr =
      Exdns.Zone.Parser.json_record_to_rr(%{
        "name" => name,
        "type" => "A",
        "ttl" => ttl,
        "data" => data
  ...
  test "json record to AAAA RR" do
    ...
    assert Exdns.Records.dns_rr(rr, :data) |> Exdns.Records.dns_rrdata_aaaa(:ip) ==
             {0, 0, 0, 0, 0, 0, 0, 1}
  end
  ...
  test "json record to CNAME RR" do
    ...
    assert Exdns.Records.dns_rr(rr, :data) |> Exdns.Records.dns_rrdata_cname(:dname) ==
             "somesite.com"
  end
  ...
  test "json record to MX RR" do
    ...
    assert Exdns.Records.dns_rr(rr, :data) |> Exdns.Records.dns_rrdata_mx(:exchange) ==
             "mx1.mail.com"
  ...
  test "json record to NS RR" do
    ...
    assert Exdns.Records.dns_rr(rr, :data) |> Exdns.Records.dns_rrdata_ns(:dname) ==
             "ns1.example.com"
  end
  ...
  test "json record to RP RR" do
    ...
    assert Exdns.Records.dns_rr(rr, :data) |> Exdns.Records.dns_rrdata_rp(:mbox) ==
             "hostmaster.example.com"

    assert Exdns.Records.dns_rr(rr, :data) |> Exdns.Records.dns_rrdata_rp(:txt) ==
             "rp.example.com"
  end
  ...
  test "json record to SOA RR" do
    ...
    assert Exdns.Records.dns_rr(rr, :data) |> Exdns.Records.dns_rrdata_soa(:mname) ==
             "ns1.example.com"

    assert Exdns.Records.dns_rr(rr, :data) |> Exdns.Records.dns_rrdata_soa(:rname) ==
             "admin.example.com"
  ...
  test "json record to SRV RR" do
    ...
    assert Exdns.Records.dns_rr(rr, :data) |> Exdns.Records.dns_rrdata_srv(:target) ==
             "x.example.com"
  end
  ...
  test "json record to SSHFP RR" do
    ...
    assert Exdns.Records.dns_rr(rr, :data) |> Exdns.Records.dns_rrdata_sshfp(:fp) ==
             <<18, 52, 86, 120, 154, 188, 222, 246, 120, 144, 18, 52, 86, 120, 154, 188, 222, 246,
               120, 144>>
  end
  ...
  test "json record to TXT RR" do
    ...
    assert Exdns.Records.dns_rr(rr, :data) |> Exdns.Records.dns_rrdata_txt(:txt) == [
             "this is some text"
           ]
  end
